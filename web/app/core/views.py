    ...
    def get_queryset(self):
        if models.Profile.objects\
            ...
            return self.queryset\
                .filter(company_id=profile.company_identificator).order_by('-last_updated')
...
class ProfileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return profiles.order_by('-last_updated')
...
class AccountViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if models.Profile.objects\
            ...
            if profile.is_admin:
                return models.Account.objects.filter(company=profile.company).order_by('-last_updated')

            return models.Account.objects\
                .filter(profile=profile, company=profile.company).order_by('-last_updated')
...
class ActionViewSet(mixins.CreateModelMixin,
    ...
    def get_queryset(self):
        if models.Profile.objects\
            ...
            return self.queryset.filter(account__in=accounts).order_by('-last_updated')
...
class TransferViewSet(mixins.CreateModelMixin,
    ...
    def get_queryset(self):
        ...
        if models.Profile.objects\
            ...
            return self.queryset.filter(from_account__in=accounts).order_by('-last_updated') \
                or self.queryset.filter(to_account__in=accounts).order_by('-last_updated')
...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if models.Profile.objects\
            ...
            return models.Category.objects.filter(company=profile.company).order_by('-last_updated')
...
class TagViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if models.Profile.objects\
            ...
            return models.Tag.objects.filter(company=profile.company).order_by('-last_updated')
